...
def home_page_layout():
    ...
    heatmap = go.Figure(data=go.Heatmap(
        ...
        text=heatmap_text,
        hoverinfo='text',
        colorscale=[[0, '#303030'], [1, '#3D9970']],  # light gray to green
        showscale=False,
    ...
    bar.update_layout(
        title={'text':'Consistency Over Last Month',
               'font':dict(size=30)},
...
def cardio_graph_time(time_weeks):
    ...
    cardio_graph.update_layout(title={'text':'Calories Burned per Cardio Session',
                                      'x':0.5,
                                      'font':dict(size=30)},
                                title_x=0.5,
                                plot_bgcolor='#303030',
